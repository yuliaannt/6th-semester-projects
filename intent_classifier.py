...
def detect_intent(data, input_vec):
    ...
    for intent in data["intents"]:
        ...
        if break_flag == 1:
            break
    if break_flag != 1 and (
        (tie_flag == 1 and intent_flag == 1 and max_score_avg < 0.06)
        or (intent_flag == 1 and max_sim_score < 0.6)
    ):
        max_sim_intent = ""
...
if __name__ == "__main__":
    ...
    input = sentence_normalizer.preprocess_main("wisata yang bagus")
    input_vec = data_embedder.embed_sentence(input, ft_model)
    output_intent = detect_intent(data, input_vec)
    print(output_intent)
